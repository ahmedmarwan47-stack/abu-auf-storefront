...
| **Gifts home section** (`GIFT_INTRO` / `GIFT_FEATURES` / `GIFT_OCCASIONS` in `build/pages/home.py`) | 2026-08-06: the gifts banner was rewritten from a single tagline + a 2×2 grid of unrelated product-spread photos to a copy-led pitch — intro paragraph, three benefit rows, occasion chips (`رمضان/الأعياد/…`). In-house Arabic, needs sign-off. The `20٪` discount figure is carried over from the old banner and is **unconfirmed** — verify or drop before launch. The hero image is `images/abuauf/site/gifts-isolated.webp`, a **real** Abu Auf gift spread (ornate dates box, sectioned nuts/dried-fruit tray, and a snack selection — pretzels, protein puffs, nut pouch, nut bar), client-supplied and background-removed with `build/isolate_products.py` (border-connected flood fill at a 222 white threshold + neutral-colour guard, so the white packaging survives and the studio checkerboard is cut to transparent). To swap in a different cut-out, drop a transparent PNG at the same path — the markup keys off the filename only |
