...
| **Gifts home section** (`GIFT_INTRO` / `GIFT_FEATURES` / `GIFT_OCCASIONS` in `build/pages/home.py`) | 2026-08-06: the gifts banner was rewritten from a single tagline + a 2×2 grid of unrelated product-spread photos to a copy-led pitch — intro paragraph, three benefit rows, occasion chips (`رمضان/الأعياد/…`). In-house Arabic, needs sign-off. The `20٪` discount figure is carried over from the old banner and is **unconfirmed** — verify or drop before launch. The hero image is `images/abuauf/site/gifts.png`, a **real** Abu Auf gift composition (dates box + nuts tray + gift box) that already ships in the repo; it keeps a soft green blend baked in rather than a fully-transparent cut-out. If a cleaner isolated cut-out is wanted, swap the file in place — the markup keys off the filename only |
