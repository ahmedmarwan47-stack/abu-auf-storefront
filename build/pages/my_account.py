    ...
    return f"""
              <a href="{href}" class="tile-lift group flex items-center gap-3 bg-white shadow-custom4 p-4 xl:p-5 rounded-[20px] hover:-translate-y-0.5">
                ...
                <div class="flex flex-col flex-1 min-w-0">
                  ...
                  <span class="font-bold text-[#062A1C] text-xl">{value_html}</span>
                </div>
    ...
    content = f"""
            ...
            <div data-email-unverified hidden class="flex flex-wrap items-center gap-3 bg-[#FFF7E6] px-5 py-4 border border-[#EAD9A0] rounded-xl">
              ...
              <span class="flex-1 min-w-0 text-sm leading-6">
                <span class="font-semibold text-[#062A1C]">أكّد بريدك الإلكتروني</span>
            ...
            <div class="gap-4 grid grid-cols-1 sm:grid-cols-3">
              {_stat("images/abuauf/icons/points-3d.png", "نقاط الولاء", f'<span class="latin" data-points-balance>{CUSTOMER["points"]:,}</span> <span class="font-medium text-neutral-secondary text-xs">نقطة</span>', "my-account-point.html")}
              {_stat("images/abuauf/icons/wallet-3d.png", "رصيد المحفظة", f'<span class="latin" data-wallet-amount>EGP {CUSTOMER["wallet"]}</span>', "my-account-wallet.html")}
              {_stat("images/abuauf/icons/orders-3d.png", "إجمالي الطلبات", f'<span class="latin">{len(ORDERS)}</span> <span class="font-medium text-neutral-secondary text-xs">طلب</span>', "my-account-orders.html")}
            </div>
