...
def build():
    ...
    steps = "".join(f"""
                <li class="flex flex-col flex-1 items-center gap-2 text-center">
                  ...
                  <span class="font-semibold text-[#062A1C] text-xs">{e(label)}</span>
    ...
    content = f"""
            <div class="flex flex-wrap justify-between items-center gap-3">
              <h1 class="font-bold text-[#062A1C] text-2xl xl:text-3xl">طلب رقم <span class="latin">#30941</span></h1>
              <span class="bg-accent-yellow px-4 py-1.5 rounded-full font-semibold text-[#062A1C] text-xs">تحت التحضير</span>
            </div>
            ...
            {card("تفاصيل الطلب", f'''<div class="flex flex-col">{lines}</div>
              <div class="flex flex-col gap-2 pt-3 border-neutral-divider border-t text-sm">
                ...
                <div class="flex justify-between"><span class="text-neutral-secondary">الإجمالي</span><span class="font-semibold text-[#062A1C] latin">EGP {money(subtotal)}</span></div>
              </div>
              <div class="flex justify-between items-center pt-3 border-neutral-divider border-t">
                <span class="font-bold text-[#062A1C]">الإجمالي</span>
    ...
    return account_page("تفاصيل الطلب | أبو عوف", "تفاصيل وحالة طلبك من أبو عوف.",
