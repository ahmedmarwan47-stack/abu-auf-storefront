...
def build():
    ...
    old_rows = "".join(f"""
                <div class="flex items-center gap-3 bg-white/70 p-4 border border-neutral-divider rounded-2xl">
                  ...
                  <span class="inline-flex items-center shrink-0 px-2.5 py-1 rounded-full font-semibold text-xs {'bg-[#E9F3E6] text-[#163300]' if used else 'bg-[#F6E9E7] text-accent-error'}">{pill}</span>
    ...
    content = f"""
            ...
            <div class="flex flex-col gap-3">
              <h2 class="font-bold text-[#062A1C] text-lg">القسائم المتاحة</h2>
            ...
            <div class="flex flex-col gap-3">
              <h2 class="font-bold text-[#062A1C] text-lg">القسائم السابقة</h2>
              <div class="flex flex-col gap-3">
                <div data-vouchers-history-dynamic class="flex flex-col gap-3"></div>
{old_rows}
              </div>
            </div>"""
    return account_page("القسائم | أبو عوف", "قسائم الخصم المتاحة في حسابك.",
